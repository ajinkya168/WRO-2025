...
def correctPosition(setPoint, head, x, y, trigger, counter, green, red, blue, orange, reset, reverse):
	...
	if not reset:
		...
		if setPoint == -50 :
			...
			if (distance_left < 25 or (distance_head < 50 and distance_left > 100)):
				correction = 10
			else:
				pass
		...
		if setPoint == 50:
			...
			if (distance_right < 25 or (distance_head < 50 and distance_left > 100)):
				correction = -10
			else:
				pass
...
def Live_Feed(color_b, stop_b, red_b, green_b, pink_b, avoid_park, centr_y):
...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y):
	...
	while True:
		print(f"Pink detetcted at counter while red is seeen: {parking_counter}")
		print(f"last_counter:{last_counter}, coutner: {counter}, parking_counter:{parking_counter}")
		...
		if pink_b.value:### DECIDES SETPOINT WHENEVER PINK IS IN THE FRAME
			...
			if orange_flag:
				...
				setPointL = -50
			pb_time = time.time()
			if counter == parking_counter and start_parking:
				r_past = False
				green_stored = False
				red_stored = False
				r_flag = False
				if not continue_parking:
		...
		if continue_parking: ### THIS SETPOINT IS WHEN THE ROBOT IS IN THE PARKING MODE
			if orange_flag:
				...
				setPointR = 30
		if orange_flag and not start_parking:
			if red_b.value:
				pink_b.value = False
				p_flag = False
				p_past = False
		...
		if setPointL == -10:
			...
			if not parking_counter_store:
				...
				print(f"Parking COunter activated.")
				parking_counter_store = True


		elif blue_flag and not start_parking:
			if green_b.value:
				pink_b.value = False
				p_flag = False
				p_past = False
		...
		try:
			...
			if button:              ##### THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
				...
				#x, y = enc.get_position(imu_head, counts.value)
					...
					if not change_path_complete:
						...
						if not change_heading:
							print(f"Changing HEadng...")
							heading_angle = heading_angle - 180
							counter = 0
							print("Changing HEadng complete")

							change_heading = True
						if abs(corr) < 10:
							print("Positive heading")
							reverse = True
							print("Positive heading complete...")
				...
				if stop_b.value:
					...
					prev_power = 0
				if p_count == 1:
					print(f"The pink block is in counter {counter}")
				...
				if parking_flag and not stop_flag:
					correctAngle(heading_angle)
					if not stop_flag and abs(corr) < 1:
						...
						while 1 and not adjust:
							...
							if(centr_y.value < 450 and pink_b.value):
								power = 0
								prev_power = 0
						...
						if orange_flag:
							...
							if not parking_heading:
								print(f"time: {time.time() - c_time}")
								print("Changing heading..")
								print(f"Before Heading_angle:{heading_angle}")
								heading_angle = heading_angle - 90
								print(f"Changing heading complete..{heading_angle}")
						...
						correctAngle(heading_angle)
						getTFminiData()
						if (abs(corr) < 5) or (distance_head < 15 and distance_right < 10 and distance_left < 10):
							power = 0
							prev_power = 0
							pwm.set_PWM_dutycycle(12, power)
							stop_flag = True
				...
				###### FOR
					...
					if reset_f:
						...
						if blue_flag:               ### BLUE RESET BLOCK
							...
							if (red_b.value or r_past) and (distance_head > 20): #red after trigger
							...
							elif (p_past or p_turn) and distance_head > 20: #pink after trigger
								p_turn = True
								red_turn = False
								print(f"Pink Detected after trigger... ")
								setPointL = 10
								...
								if counter == last_counter and green_count == 1:
									change_path = False
									power = 60

								elif counter == last_counter and red_count == 1:
									change_path = True
									power = 60
									reset_f = False
						...
						if orange_flag:         ### ORANGE RESET BLOCK
							...
							if ((g_flag or green_turn) and distance_head > 20): #green after trigger
								...
								print(f"pink_b.value: {pink_b.value}, p_flag: {p_flag}, p_past:{p_past}, g_past:{g_past}")
								green_turn = True
								setPointL = -10
					...
					if green_b.value and (not r_flag and not p_flag):
						...
						if distance_right < 45 and g_past:
							g_past = False
							green_stored = False
							g_flag = False
							#setPointL = -80
						...
						getTFminiData()
						# print(f"distance_left:{distance_left}")
						if distance_left < 45 and r_past:
							...
							rp_time = time.time()
						print('4')
					elif pink_b.value and (not g_flag and not r_flag):
						p_flag = True
						p_past = True
						if start_parking:
							g_past = False
							...
							r_flag = False
						print('5')
					elif not pink_b.value and p_past:
						p_flag = True
						getTFminiData()
						if (distance_left <= 20 and distance_right < 65) or time.time() - pp_time < 0.5:
							# print(f"red Avoid complete")
							p_past = False
							p_flag = False
							parking_counter = counter
							print("Pink Avoi dance Complete...")
							pp_time = time.time()
							if not parking_counter_store:
								parking_counter = counter
								print(f"Pink detetcted at counter while pink is seen: {parking_counter}")
								parking_counter_store = True
							if not parking_flag and counter == parking_counter and continue_parking:
								parking_flag = True
						print('6')
...
if __name__ == '__main__':
	try:
		...
		P = multiprocessing.Process(target=Live_Feed, args=(
			color_b, stop_b, red_b, green_b, pink_b, avoid_park, centr_y,))
		S = multiprocessing.Process(target=servoDrive, args=(
			pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y,))
