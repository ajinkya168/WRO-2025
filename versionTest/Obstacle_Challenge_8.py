...
def Live_Feed(color_b, stop_b, red_b, green_b, pink_b, centr_y, centr_x, centr_y_red, centr_x_red, centr_x_pink, centr_y_pink, centr_y_b, blue_b, prev_b, orange_o, centr_y_o):
	...
	cv2.resizeWindow('Object Frame', 200, 400)
	time.sleep(2)
	GPIO.output(green_led, GPIO.HIGH)
	...
	while True:
		...
		# For Green Color
		lower = np.array([66, 35, 30])  # green
		upper = np.array([90, 230, 110])
		mask = cv2.inRange(hsv_img, lower, upper)

		# For Red Color
		lower1 = np.array([0, 110, 47])  # red
		upper1 = np.array([4, 245, 112])
		r1lower = np.array([170, 110, 47])
		r1upper = np.array([179, 245, 112])
...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, pink_b, counts, centr_y, centr_x, centr_y_red, centr_x_red, centr_x_pink, centr_y_pink, head, centr_y_b, blue_b, prev_b, orange_o, centr_y_o, blue_c, orange_c, white_c):
	...
	color_s = ""
	while True:
		if centr_y_b.value > 500 and (counter == 0 or blue_flag):
			power = 50
		...
		if continue_parking:  ### THIS FLAG SETS TO TRUE IS WHEN THE ROBOT IS IN THE PARKING MODE
			green_b.value = False
			red_b.value = False
			g_past = False
			r_past = False
		...
		while True:
			...
			try:
				...
				if data[0].isdigit() or data[1].isdigit():
					...
					#print(f"counts:{counts.value} imu:{head.value}")
				else:
					pass
			except ValueError:
				continue
