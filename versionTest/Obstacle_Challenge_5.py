        ...
        for i in range(0, count_right - 9):
            if recv_right[i] == 89 and recv_right[i + 1] == 89:  # 0x59 is 89
                ...
                if checksum == recv_right[i + 8]:
                    global distance_right
                    distance_right = recv_right[i +
                                                2] + recv_right[i + 3] * 256
                    strength_right = recv_right[i +
                                                4] + recv_right[i + 5] * 256
...
def correctPosition(setPoint, head, x, y, trigger, counter, green, red, blue, orange, reset, reverse):
    ...
    if not reset:
        ...
        if setPoint == 70:
            ...
            if (distance_right < 25) or (distance_head < 70 and distance_left < 30):
...
def Live_Feed(color_b, stop_b, red_b, green_b, pink_b, avoid_park, centr_y, centr_y_red, centr_y_pink, g_next, r_next):
    ...
    while True:
        ...
        d_img = cv2.morphologyEx(mask, cv2.MORPH_OPEN,
                                 kernel, iterations=5)  # green
        d_img1 = cv2.morphologyEx(
            mask1, cv2.MORPH_OPEN, kernel, iterations=5)  # red
        d_img2 = cv2.morphologyEx(
            mask2, cv2.MORPH_OPEN, kernel, iterations=5)  # pink
...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y, centr_y_red, centr_y_pink, g_next, r_next):
    ...
    while True:
        ...
        if (pink_b.value):  # DECIDES SETPOINT WHENEVER PINK IS IN THE FRAME
            ...
            if orange_flag:
                ...
                if not pink_and_red:
                    pink_and_red = True
            pb_time = time.time()
            if start_parking and not continue_parking:
                continue_parking = True
        # IF DOES NOT SEE PINK, KEEP THE SAME SETPOINT FOR 1 SECOND AND THEN CHANGE
        elif (not pink_b.value and time.time() - pb_time > 1):
            setPointL = -70
            setPointR = 70
        else:
            pass

        if not prev_distance_store and reset_f:
            getTFminiData()
        ...
        try:
            ...
            if button:  # THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
                ...
                if not change_path:
                    imu_head = glob
                elif change_path and not reset_all:
                ...
                if parking_flag and not stop_flag:
                    ...
                    print(
                        f"distance_head : {distance_head}, distance_left:{distance_left}")
                ...
                # FOR
                    ...
                    if reset_f:
                        ...
                        if blue_flag:  # BLUE RESET BLOCK
                            ...
                            if (red_b.value or r_past) and (distance_head > 20):  # red after trigger
                                ...
                                p_turn = False
                                setPointR = 10
                                correctPosition(setPointR, heading_angle, x, y, trigger, counter,
                                                g_flag, r_flag, blue_flag, orange_flag, reset_f, reverse)
                        ...
                        if orange_flag:  # ORANGE RESET BLOCK
                            ...
                            if ((green_b.value or green_turn) and distance_head > 20):  # green after trigger
                                ...
                                correctPosition(setPointL, heading_angle, x, y, trigger, counter,
                                                g_flag, r_flag, blue_flag, orange_flag, reset_f, reverse)
                            else:
                                setPointL = -70
                                if green_turn:
                    ...
                    if (green_b.value and (not r_flag and not p_flag)) and not continue_parking:
                        g_flag = True
                        g_past = True
				...
				parking_counter_store = True'''
                        ...
                        print('2')
                    elif (red_b.value and (not g_flag and not p_flag)) and not continue_parking:
                        r_flag = True
                        r_past = True
                        '''if pink_and_green or pink_and_red:
			    if not parking_counter_store:
				...
				parking_counter_store = True'''
                        ...
                        p_flag = True
                        getTFminiData()
                        if distance_left <= 20 and (prev_distance - distance_left > 20) and p_past:
                            # print(f"red Avoid complete")
                            p_past = False
                            p_flag = False
                            pink_avoid = True
                    ...
                    if g_flag:
                        ...
                        correctPosition(0, heading_angle, x, y, trigger, counter,
                                        g_flag, r_flag, blue_flag, orange_flag, reset_f, reverse)
                ...
                print(
                    f" color: {color_sensor}, orange:{orange_flag}, blue:{blue_flag}, corr:{corr}")
...
if __name__ == '__main__':
    try:
        ...
        P = multiprocessing.Process(target=Live_Feed, args=(
            color_b, stop_b, red_b, green_b, pink_b, avoid_park, centr_y, centr_y_red, centr_y_pink, g_next, r_next,))
        S = multiprocessing.Process(target=servoDrive, args=(
            pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y, centr_y_red, centr_y_pink, g_next, r_next))
